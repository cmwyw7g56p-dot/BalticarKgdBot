...
def day_has_available_return(
    ...
    """
    День возврата зелёный только если хотя бы одно время возврата
    формирует полностью свободный интервал start_at -> end_at.

    Важный момент: проверяется ВЕСЬ период, а не только последний час.
    Поэтому нельзя построить бронь 08.10 -> 21.10 поверх существующей
    аренды 09.10 -> 20.10.
    """
    start_at = ensure_tz(start_at)
    ...
    for n in range(1, days + 1):
        ...
        if mode == "start":
            ...
            if start_at is None:
                ...
                available_day = day_has_available_return(
                    current,
                    start_at,
                    bookings,
                    exclude_booking_id=bid
